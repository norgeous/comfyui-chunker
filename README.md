...
- Easily blend overlaps of latents, images, masks and audio, with a variety of blend methods.
...
Repeat nodes between this node and `🍫 Combine`. Optionally divide long batches of images, masks, audio and / or latents into smaller chunks and process the chunks sequentially. Optionally use the end of last chunk as start of this chunk (with `overlap_length`).
...
Combine sequential chunks back into single av tensors. Each chunk is saved to a mp4 file (and latents to safetensors) and recombined excluding the overlap frames.
