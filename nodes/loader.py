...
def load_video_chunk(video_path, start_n=0, end_n=None):
...
def awesome_loader(path, start, end):
    ...
    if file_ext in img_ext:
        # image_path = folder_paths.get_annotated_filepath(image)
        image, mask = load_image_advanced(path)
        fps = 0
        total_length = 1
        return (image, mask, fps, total_length)
    if file_ext in vid_ext:
        frames, fps, total_length = load_video_chunk(path, start_n=start, end_n=end)
        mask = None
        return (frames, mask, fps, total_length)
